...
void displayName() {
    ...
    QWORD len = 1;
    STATUS status1 = SyscallProcessGetName(processName, len);
    LOG_TRACE_USERMODE("Status: %d\nFor ProcessNameMaxLen=%d, process name is: %s", status1, len, processName);

    len = 3;
    STATUS status2 = SyscallProcessGetName(processName, 3);
    LOG_TRACE_USERMODE("Status: %d\nFor ProcessNameMaxLen=%d, process name is: %s", status2, len, processName);
    
    len = 0x1234;
    STATUS status3 = SyscallProcessGetName(processName, len);
    LOG_TRACE_USERMODE("Status: %d\nFor ProcessNameMaxLen=%d, process name is: %s", status3, len, processName);
}

void displayPriority() {

    BYTE currentPriority = 0;
    STATUS status = SyscallGetThreadPriority(currentPriority);
    LOG_TRACE_USERMODE("Status: %d, Current priority: %d", status, currentPriority);

    currentPriority++;
    status = SyscallSetThreadPriority(currentPriority);
    LOG_TRACE_USERMODE("Status: %d", status);

    status = SyscallGetThreadPriority(currentPriority);
    LOG_TRACE_USERMODE("Status: %d, Current priority: %d", status, currentPriority);
}

void displayNumberOfThreadsStarted() {
    
    QWORD* threadNo = NULL;
    STATUS status = SyscallGetNumberOfThreadsForCurrentProcess(threadNo);
    PCPU* pcpu = GetCurrentPcpu();
    LOG_TRACE_USERMODE("Status: %d, Number: %d, CPU ID: %d", status, &threadNo, pcpu->ApicId);
}